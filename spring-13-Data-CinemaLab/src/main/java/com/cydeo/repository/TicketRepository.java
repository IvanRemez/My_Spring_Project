...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    Integer countAllByUserAccountId(Long userAccountId);
    ...
    @Query(value = "SELECT DISTINCT m.name FROM ticket t JOIN movie_cinema mc " +
            "ON t.movie_cinema_id = mc.id " +
            "JOIN movie m ON mc.movie_id = m.id", nativeQuery = true)
    List<String> fetchAllDistinctMovieNames();
                                                                        // *****
    ...
    List<Ticket> fetchAllTicketsByUserEmail(String email);
